...
class RecommendationCache:
    ...
    # Redis key prefixes
    PREFIX_USER_REC = "rec:user"           # 用户个性化推荐
    PREFIX_TRENDING = "trending"           # 热门视频
    PREFIX_TRENDING_CATEGORY = "trending:cat"  # 分类热门
    PREFIX_VIDEO_STATS = "video:stats"      # 视频统计数据
    ...
    CACHE_CONFIG = {
        ...
        "trending": 1800,                   # 30分钟
        "personalized": 86400,              # 24小时（用户个性化推荐）
    }
    ...
    def zadd_trending(self, video_id: str, score: float, ttl: int = 86400) -> bool:
        """添加视频到全局热门列表"""
        try:
            key = f"{self.PREFIX_TRENDING}:global"
            self.redis_client.zadd(key, {video_id: score})
            self.redis_client.expire(key, ttl)
            return True
        except Exception as e:
            logger.error(f"❌ zadd_trending 失败: {e}")
            return False

    def zincrby_trending(self, video_id: str, increment: float) -> bool:
        """热门视频分数增量更新（实时）"""
        try:
            key = f"{self.PREFIX_TRENDING}:global"
            self.redis_client.zincrby(key, increment, video_id)
            return True
        except Exception as e:
            logger.error(f"❌ zincrby_trending 失败: {e}")
            return False

    def zadd_trending_category(self, category_id: int, video_id: str, score: float, ttl: int = 86400) -> bool:
        """添加视频到分类热门列表"""
        try:
            key = f"{self.PREFIX_TRENDING_CATEGORY}:{category_id}"
            self.redis_client.zadd(key, {video_id: score})
            self.redis_client.expire(key, ttl)
            return True
        except Exception as e:
            logger.error(f"❌ zadd_trending_category 失败: {e}")
            return False

    def zincrby_trending_category(self, category_id: int, video_id: str, increment: float) -> bool:
        """分类热门分数增量更新"""
        try:
            key = f"{self.PREFIX_TRENDING_CATEGORY}:{category_id}"
            self.redis_client.zincrby(key, increment, video_id)
            ...
            logger.error(f"❌ zincrby_trending_category 失败: {e}")
            return False

    def zrange_trending(self, start: int = 0, end: int = 99, withscores: bool = True) -> list:
        """获取全局热门列表"""
        try:
            key = f"{self.PREFIX_TRENDING}:global"
            if withscores:
                return self.redis_client.zrevrange(key, start, end, withscores=True)
            else:
                return self.redis_client.zrevrange(key, start, end)
        except Exception as e:
            logger.error(f"❌ zrange_trending 失败: {e}")
            return []

    def zrange_trending_category(self, category_id: int, start: int = 0, end: int = 49, withscores: bool = True) -> list:
        """获取分类热门列表"""
        try:
            key = f"{self.PREFIX_TRENDING_CATEGORY}:{category_id}"
            if withscores:
            ...
            logger.error(f"❌ zrange_trending_category 失败: {e}")
            return []

    # ==================== Sorted Set 操作（用户个性化推荐） ====================

    def zadd_user_recommendations(self, user_id: str, recommendations: list, ttl: int = 86400) -> bool:
        """
        设置用户个性化推荐（Sorted Set）
        recommendations: [(video_id, score), ...]
        """
        try:
            key = f"{self.PREFIX_USER_REC}:{user_id}"
            # 先删除旧数据
            self.redis_client.delete(key)
            if recommendations:
                # 转换为 dict 格式
                data = {str(vid): float(score) for vid, score in recommendations}
                self.redis_client.zadd(key, data)
                self.redis_client.expire(key, ttl)
            return True
        except Exception as e:
            logger.error(f"❌ zadd_user_recommendations 失败: {e}")
            return False

    def zrange_user_recommendations(self, user_id: str, start: int = 0, end: int = 49, withscores: bool = True) -> list:
        """获取用户个性化推荐"""
        try:
            key = f"{self.PREFIX_USER_REC}:{user_id}"
            if withscores:
            ...
            logger.error(f"❌ zrange_user_recommendations 失败: {e}")
            return []

    def zincrby_user_recommendations(self, user_id: str, video_id: str, increment: float) -> bool:
        """增量更新用户推荐分数（用户行为触发）"""
        try:
            key = f"{self.PREFIX_USER_REC}:{user_id}"
            self.redis_client.zincrby(key, increment, video_id)
            ...
            logger.error(f"❌ zincrby_user_recommendations 失败: {e}")
            return False
    ...
    def delete_user_recommendations(self, user_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"❌ delete_user_recommendations 失败: {e}")
            return False

    def delete_trending_video(self, video_id: str) -> bool:
        """从热门列表删除视频"""
        try:
            # 全局热门
            key = f"{self.PREFIX_TRENDING}:global"
            self.redis_client.zrem(key, video_id)
            # 所有分类热门
            pattern = f"{self.PREFIX_TRENDING_CATEGORY}:*"
            for key in self.redis_client.scan_iter(pattern):
                self.redis_client.zrem(key, video_id)
            return True
        except Exception as e:
            logger.error(f"❌ delete_trending_video 失败: {e}")
            return False
