    ...
    async def get_collaborative_recommendations(self, limit: int = 10) -> List[Tuple[UUID, float]]:
        ...
        try:
            ...
            stmt = (
                ...
                .where(~UserVideoHistory.video_id.in_(user_watched))
                .where(Video.is_approved == "approved")
    ...
    async def get_recommendations_for_slot(
        ...
        try:
                ...
                # manual_first fallback 到热门推荐，或者有 category_id 时也需要过滤
                    ...
                    for score_record in cached_scores:
                        if score_record.video_id not in exclude_video_ids and score_record.video_id not in recommendations:
                            # 检查视频是否有效（未下架、已审核、已上线）
                            video = self.session.exec(
                                select(Video).where(Video.id == score_record.video_id)
                            ).first()
                            if not video or video.is_deleted or video.is_approved != "approved" or video.visibility != "public" or video.status != "completed":
                                continue
                            # 根据缓存记录确定推荐来源
            ...
            for rank, (video_id, info) in enumerate(list(recommendations.items())[:limit], 1):
                ...
                if video and not video.is_deleted and video.is_approved == "approved" and video.visibility == "public" and video.status == "completed":
                    result.append({
                        "video_id": video_id,
                        "score": info["score"],
                        "source": info["source"],
...
async def compute_user_recommendation_scores(session: Session, user_id: UUID) -> int:
    ...
    try:
        ...
        # 保存到 Redis Sorted Set
        cache = get_cache()
        user_id_str = str(user_id)

        if final_scores:
            # 存 Redis（top 100）
            cache.zadd_user_recommendations(user_id_str, final_scores[:100])
        else:
            # 无结果时清除缓存
            cache.delete_user_recommendations(user_id_str)
